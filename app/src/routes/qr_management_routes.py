...
@qr_mgmt_bp.route('/print-layout', methods=['POST'])
@login_required
@admin_required
def generate_print_layout():
    """Generate a printable HTML layout for multiple QR codes."""
    try:
        data = request.get_json()
        qr_ids = data.get('qr_ids', [])
        codes_per_page = int(data.get('codes_per_page', 9))
    except Exception:
        return jsonify({'success': False, 'message': 'Invalid data'})
        
    if not qr_ids:
        return jsonify({'success': False, 'message': 'No QR codes selected'})
        
    # Get required QR codes
    students = db._execute(
        f"SELECT school_id, name, qr_data_encoded, course, year_level, section FROM students_qrcodes WHERE school_id IN ({','.join(['?']*len(qr_ids))})",
        tuple(qr_ids), fetch_all=True
    ) or []
    
    selected_codes = [{
        'school_id': row[0], 'name': row[1], 'qr_data_encoded': row[2],
        'course': row[3], 'year_level': row[4], 'section': row[5]
    } for row in students]
    
    html_content = render_template('qr/print_layout.html', codes=selected_codes, per_page=codes_per_page)
