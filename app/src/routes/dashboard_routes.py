...
def home():
    ...
    # Get total students for progress bars
    total_students_res = db._execute("SELECT COUNT(*) FROM students_qrcodes", fetch_all=True)
    total_students = total_students_res[0][0] if total_students_res and total_students_res[0] else 0

    # Get active events (today)
    today_str = datetime.now().strftime("%Y-%m-%d")
    active_events = [e for e in events if e[2] == today_str]
    
    # Enrich active events with scan counts
    for idx, e in enumerate(active_events):
        event_id = e[0]
        # Count unique students scanned for this event
        scanned_count_res = db._execute(
            "SELECT COUNT(DISTINCT user_id) FROM attendance WHERE event_id = ?",
            (event_id,), fetch_all=True)
        scans = scanned_count_res[0][0] if scanned_count_res and scanned_count_res[0] else 0
        active_events[idx] = list(e) + [scans, total_students]
