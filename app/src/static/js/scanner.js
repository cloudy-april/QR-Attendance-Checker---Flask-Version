...
function toggleFullscreen() {
    ...
    container.classList.toggle('scanner-fullscreen');
    
    if (container.classList.contains('scanner-fullscreen')) {
...
function provideHapticFeedback(type) {
    if (navigator.vibrate) {
        if (type === 'success') {
            ...
            navigator.vibrate([100, 50, 100, 50, 100]); // Vibrates 3 times for error
        }
    }
}
...
function flashScreen(type) {
    ...
    if (overlay) {
        overlay.className = 'scan-feedback-overlay ' + type;
        
        // Remove and re-add class to trigger animation if it's already active
...
function markAttendance(qrData) {
    ...
    .then(response => {
        ...
        if (response.success) {
            ...
            indicator.innerHTML = `<div style="font-size: 1.5em; font-weight: bold; margin-bottom: 5px;">${response.user_name}</div><div>Marked for ${selectedTimeSlot}</div>`;
            indicator.style.display = 'block';
